	...
	$.Stapel.prototype = {
			...
			for( var pile in this.piles ) {
				...
				for( var i = 0, len = p.elements.length; i < len; ++i ) {
					...
					if( i === len - 2 ) {
						...
						$el.data( 'front', true ).append( '<div class="pport-title"><span>' + pile + '</span><span>' + len + '</span></div>' );
					}
			...
			for( var pile in this.piles ) {
				...
				for( var i = 0, len = p.elements.length; i < len; ++i ) {
					...
					if( pile === this.pileName ) {
						...
						if( $item.data( 'front' ) ) {
							$item.find( 'div.pport-title' ).hide();
						}
		...
		_closePile : function() {
			...
			if( this.spread ) {
				...
				this.el.removeClass( 'pport-open' );
